...
<% for quantity in r.get('Dom_significant_quantities_labels', []) %>
<< quantity >><% if loop.revindex0 == 1 %> and <% elif not loop.last %>, <% else %>. <% endif %>
<% endfor %>

<% for item in r.get('Dom_distribution_summary_by_sample', []) %>
<% if item.has_significant_difference %>
In << item.sample_label >>, the strongest corrected dominated/non-dominated separation concerns << item.quantity_label >>:
the dominated groups have a median value of << item.median_dom_fmt >>, compared to << item.median_nondom_fmt >> for the non-dominated ones
...
For << item.sample_label >> << item.domination|lower >> groups, $S_\mathrm{frac}$ correlates with << item.quantity_label >>
