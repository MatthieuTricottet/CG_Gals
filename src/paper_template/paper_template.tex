...
% \begin{table} \label{tab:sample_sizes}
%     \centering
%     \begin{tabular}{lccc}
%         \toprule
%         \textbf{Sample} & \textbf{$N_{AGN}$} & \textbf{$N_{AGN,(z,R)}$} & \textbf{$N_{(z,R)}$ }\\
%         \midrule
%         \CG &  <<CG3_500_Ngal>>  &  <<CG3_500_Rzcut_Ngal>>  &  <<CG3_1000_Ngal>>  \\
%         $CG_{3,1000}$ &  <<CG3_1000_Ngal>>  &  <<CG3_1000_Rzcut_Ngal>>  &  <<CG3_1000_Ngal>>  \\
%         $CG_{4,500}$ &  <<CG4_500_Ngal>>  &  <<CG4_500_Rzcut_Ngal>>  &  <<CG4_500_Ngal>>  \\
%         $CG_{4,1000}$ &  <<CG4_1000_Ngal>>  &  <<CG4_1000_Rzcut_Ngal>>  &  <<CG4_1000_Ngal>>  \\
%         Control &  <<SDSS_init_Ngal>>   &  <<SDSS_init_Ngal>>  &  <<Control_Ngal>>  \\
%         \bottomrule
%     \end{tabular}
%     \caption{Number of galaxies in each sample.}
% \end{table}    


We select non-AGN galaxies by first placing them on the classical BPT diagnostic of \cite{Veilleux87} :contentReference[oaicite:0]{index=0} and requiring measured values of both emission-line ratios
\[
...
\]
in which case it is removed from the non-AGN sample :contentReference[oaicite:1]{index=1}.  All remaining galaxies—including those with missing line ratios—are retained as non-AGN :contentReference[oaicite:2]{index=2}.
The non-AGN selection process is shown on figure \ref{fig:BTP}.


\begin{figure}
    \centering
     \includegraphics[width=\columnwidth]{BPT_diagram.pdf}
     \caption{BPT diagram showing our separation of ordinary and AGN galaxies.}
     \label{fig:BPT}
\end{figure}


% We identify the galaxies having a \texttt{specsf\_tot\_p50} of -9999 in the SDSS as quenched. We then consider the other ones and sort them between the 
...
% The sSFR status for each sample is shown in table \ref{tab:sSFR_status}. The probability for \<<sSFR_status[2]>> {} proportion to be different form the control sample's 
% one is, estimated using Fisher exact test, is  <<pval_3_1000_StarformingvsControl>>  for the \CGto{} sample, 
%  <<pval_3_500_StarformingvsControl>>  for \CGtf,  <<pval_4_1000_StarformingvsControl>>  for \CGfo{} and  <<pval_3_500_StarformingvsControl>>  for \CGff.
...
%             $CG_{3,1000}$ &  <<CG3_1000_NQuenched>>  (<<CG3_1000_NQuenched_pc>> \%) &  <<CG3_1000_NPassive>>  (<<CG3_1000_NPassive_pc>> \%)&  <<CG3_1000_NStarforming>>  (<<CG3_1000_NStarforming_pc>> \%) \\
%             $CG_{3,500}$ &  <<CG3_500_NQuenched>>  (<<CG3_500_NQuenched_pc>> \%) &  <<CG3_500_NPassive>>  (<<CG3_500_NPassive_pc>> \%) &  <<CG3_500_NStarforming>>  (<<CG3_500_NStarforming_pc>> \%) \\
%             $CG_{4,1000}$ &  <<CG4_1000_NQuenched>>  (<<CG4_1000_NQuenched_pc>> \%) &  <<CG4_1000_NPassive>>  (<<CG4_1000_NPassive_pc>> \%) &  <<CG4_1000_NStarforming>>  (<<CG4_1000_NStarforming_pc>> \%) \\
%             $CG_{4,500}$ &  <<CG4_500_NQuenched>>  (<<CG4_500_NQuenched_pc>> \%) &  <<CG4_500_NPassive>>  (<<CG4_500_NPassive_pc>> \%) &  <<CG4_500_NStarforming>>  (<<CG4_500_NStarforming_pc>> \%)\\
%             Control &  <<Control_NQuenched>>  (<<Control_NQuenched_pc>> \%) &  <<Control_NPassive>>  <<Control_NPassive_pc>> \%) &  <<Control_NStarforming>>   (<<Control_NStarforming_pc>> \%) \\
%          \midrule
%         \bottomrule
%     \end{tabular}
...
\begin{figure}
    \centering
     ...
     \caption{sSFR vs mass, star-forming/passive limit and morphologies for the SDSS control sample.}
...
%      \includegraphics[width=\columnwidth]{residual_sSFR_histogram.pdf}
%      \caption{}
%      \label{fig:residual_sSFR_histogram}
% \end{figure}
...
% Other cases are considered as ``uncertain''. 

% Once galaxies having uncertain morphology removed, \CG{} have <<Nb_S_CG>> (<<fraction_S_CG_pc>>) of their galaxies classified as spiral and 
% <<Nb_E_CG>> (<<fraction_E_CG_pc>>) elliptical,
% while the control sample has <<Nb_S_Control>> (<<fraction_S_Control_pc>>) and <<Nb_E_Control>> (<<fraction_E_Control_pc>>), respectively. 
% The probability that those fractions come from 
% the same distribution is astimated as <<pval_morphologies_CGvsControl_pc>> from the Fisher two-sided exact test.
